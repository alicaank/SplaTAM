...
from utils.slam_helpers import (
    transformed_params2rendervar, transformed_params2depthplussilhouette, transform_to_frame, l1_loss_v1, matrix_to_quaternion,
    ce_loss
)
from utils.slam_external import calc_ssim, build_rotation, prune_gaussians, densify

from diff_gaussian_rasterization import GaussianRasterizer as Renderer
from light_conv.lang_encoder import LightweightConvLayer, EmbeddingLayer
...
def initialize_params(init_pt_cld, num_frames, mean3_sq_dist, gaussian_distribution, include_feature = False):
    ...
    if include_feature:
        params = {
            ...
            'language_feature': torch.zeros((init_pt_cld[:, 3:6].shape[0], 3), device="cuda"),
        ...
        params = {
            'means3D': means3D,
            'rgb_colors': init_pt_cld[:, 3:6],
            'language_feature': torch.zeros((1,), dtype=torch.float, device="cuda"),
            'unnorm_rotations': unnorm_rots,
            'logit_opacities': logit_opacities,
            'log_scales': log_scales,
        }
...
def initialize_optimizer(params, lrs_dict, include_feature):
    lrs = lrs_dict
    param_groups = [{'params': [v], 'name': k, 'lr': lrs[k]} for k, v in params.items()]
    if not include_feature:
        param_groups = [pg for pg in param_groups if pg['name'] != 'language_feature']
...
def initialize_first_timestep(dataset, num_frames, scene_radius_depth_ratio, 
    ...
    # Setup Camera
    cam = setup_camera(color.shape[2], color.shape[1], intrinsics.cpu().numpy(), w2c.detach().cpu().numpy(), include_feature = include_feature)
    ...
    params, variables = initialize_params(init_pt_cld, num_frames, mean3_sq_dist, gaussian_distribution, include_feature = include_feature)
...
def get_loss(params, curr_data, variables, iter_time_idx, loss_weights, use_sil_for_loss,
             ...
             language_feature = None, language_feature_mask = None, include_feature = False, decoder = None):
    ...
    im, rendered_language_feature, radius, _, = Renderer(raster_settings=curr_data['cam'])(**rendervar)
    ...
    # Language Loss
    if include_feature:
        expand_render_lang = curr_data['decoder'](rendered_language_feature)
        losses['language'] =  ce_loss(expand_render_lang, language_feature, language_feature_mask)
...
def initialize_new_params(new_pt_cld, mean3_sq_dist, gaussian_distribution, include_feature = False):
    ...
    if gaussian_distribution == "isotropic":
        ...
        raise ValueError(f"Unknown gaussian_distribution {gaussian_distribution}")
    if include_feature:
        # language_feature_precomp = new_pt_cld[:, 6:9]
        # language_feature_precomp = language_feature_precomp/ (language_feature_precomp.norm(dim=-1, keepdim=True) + 1e-9)
        params = {
            'means3D': means3D,
            'rgb_colors': new_pt_cld[:, 3:6],
            'unnorm_rotations': unnorm_rots,
            'language_feature': torch.zeros((new_pt_cld[:, 3:6].shape[0], 3), device="cuda"), #language_feature_precomp,
            'logit_opacities': logit_opacities,
            'log_scales': log_scales,
        }
    else:
...
def add_new_gaussians(params, variables, curr_data, sil_thres, 
                      time_idx, mean_sq_dist_method, gaussian_distribution, include_feature = False):
    ...
    if torch.sum(non_presence_mask) > 0:
        ...
        new_params = initialize_new_params(new_pt_cld, mean3_sq_dist, gaussian_distribution, include_feature = include_feature)  
...
def rgbd_slam(config: dict):
    ...
    if config['use_wandb']:
        ...
        wandb.login(key="79895c54bff2fc3c6ec637e5a864bd0f5011e1af")
    ...
    if seperate_densification_res:
        ...
        gt_language_feature, language_feature_mask, num_classes = dataset.get_language_feature(3, 0)
        decoder = LightweightConvLayer(num_classes,3).to(device)
    ...
    if seperate_tracking_res:
        ...
        tracking_color, _, _, tracking_intrinsics, _ = tracking_dataset[0]
        ...
        tracking_cam = setup_camera(tracking_color.shape[2], tracking_color.shape[1], 
                                    tracking_intrinsics.cpu().numpy(), first_frame_w2c.detach().cpu().numpy(), include_feature = include_feature)
    ...
    if config['load_checkpoint']:
        ...
        for time_idx in range(checkpoint_time_idx):
            ...
            gt_language_feature, gt_language_feature_mask, num_classes = dataset.get_language_feature(3, time_idx)
            decoder = LightweightConvLayer(num_classes,3).to(device)
            # Process poses
            ...
            if time_idx in keyframe_time_indices:
                ...
                curr_keyframe = {'id': time_idx, 'est_w2c': curr_w2c, 'color': color, 'depth': depth, 'language': gt_language_feature, 'language_mask': language_feature_mask}
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        gt_language_feature, language_feature_mask, num_classes = dataset.get_language_feature(3, time_idx)
        decoder = LightweightConvLayer(num_classes,3).to(device)
        curr_data = {'cam': cam, 'im': color, 'depth': depth, 'id': iter_time_idx, 'intrinsics': intrinsics, 
                    'w2c': first_frame_w2c, 'iter_gt_w2c_list': curr_gt_w2c,
                    'gt_language_feature': gt_language_feature, 'language_feature_mask': language_feature_mask,
                    'decoder': decoder}
        ...
        if seperate_tracking_res:
            ...
            tracking_curr_data = {'cam': tracking_cam, 'im': color, 'depth': depth, 'id': iter_time_idx,
        ...
        if time_idx > 0 and not config['tracking']['use_gt_poses']:
            ...
            optimizer = initialize_optimizer(params, config['tracking']['lrs'], include_feature=include_feature)
            ...
            while True:
                ...
                loss, variables, losses = get_loss(params, tracking_curr_data, variables, iter_time_idx, config['tracking']['loss_weights'],
                                                   ...
                                                   tracking_iteration=iter, include_feature = include_feature, language_feature = gt_language_feature, language_feature_mask = language_feature_mask)
                if config['use_wandb']:
                    # Report Loss
        ...
        if time_idx == 0 or (time_idx+1) % config['map_every'] == 0:
            ...
            optimizer = initialize_optimizer(params, config['mapping']['lrs'], include_feature=include_feature) 
            ...
            for iter in range(num_iters_mapping):
                ...
                if selected_rand_keyframe_idx == -1:
                    ...
                    iter_language= keyframe_list[selected_rand_keyframe_idx]['language']
                    iter_language_feature_mask = keyframe_list[selected_rand_keyframe_idx]['language_mask'] 
                    iter_decoder = keyframe_list[selected_rand_keyframe_idx]['decoder']
                ...
                loss, variables, losses = get_loss(params, iter_data, variables, iter_time_idx, config['mapping']['loss_weights'],
                                                ...
                                                config['mapping']['use_l1'], config['mapping']['ignore_outlier_depth_loss'],  tracking = False, 
                                                mapping=True, language_feature = iter_language, language_feature_mask = iter_language_feature_mask, 
                                                include_feature = include_feature)
                if config['use_wandb']:
                    # Report Loss
        ...
        if ((time_idx == 0) or ((time_idx+1) % config['keyframe_every'] == 0) or \
            ...
            with torch.no_grad():
                ...
                curr_keyframe = {'id': time_idx, 'est_w2c': curr_w2c, 'color': color, 'depth': depth, 
                                 'language': gt_language_feature, 'language_mask': language_feature_mask,
                                 'decoder': decoder}
                # Add to keyframe list
                keyframe_list.append(curr_keyframe)
